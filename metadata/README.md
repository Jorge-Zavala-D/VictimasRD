...
The `rd-outcomes/` subdirectory contains the non-observation outcome registry
for the canonical estimation modules. `rd-outcome-output-manifest.csv` records
the generator, logical input signature, run identifier, checksum, and review
status of every aggregate table and figure generated by the 2013 CCPP module.
All listed artifacts remain `generated_unreviewed` until substantive,
disclosure, and manuscript review is complete.

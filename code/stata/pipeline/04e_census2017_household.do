/*
Project: Victimas RD
Purpose: Estimate Census 2017 household reduced-form and fuzzy-RD effects
Unit:    SISFOH source household followed into the INEI-assisted 2017 cohort
...
capture log close victimasrd_rd_2017_household
log using "${logs_root}/rd_2017_household_${rd_run_id}.smcl", ///
    name(victimasrd_rd_2017_household) replace
...
program define _vrd_prepare_household_outcomes
    version 19
    confirm variable hh_share_moved_ccpp_2017
    confirm variable hh_wellbeing_core_2017
end

use "${rd_input_2017_household}", clear
...
local required_vars ///
    census2017_baseline_hhid ruv_id ubigeo_dist ubigeo_ccpp ///
    sample_main_rd victimization_level_source ///
    ${rd_running} ${rd_treatment_2017} ///
    census2017_cohort_members hh_link_rate_2017 ///
...
_vrd_post_household_rd, ///
    ...
    outcomeid("D01") ///
    outlabel("Treatment through 2016: source-household universe") ///
...
_vrd_post_household_rd, ///
    ...
    outvar(${rd_treatment_2017}) ///
    outcomeid("D03") ///
    outlabel("Treatment through 2016: household-equal sensitivity") ///
    family("design") tier("design") multiplicity("design") ///
...
_vrd_post_household_rd, ///
    posthandle(`rd_post') ///
    outvar(rd_primary_eligible) ///
    outcomeid("D05") ///
    outlabel("Household included in complete primary sample") ///
    family("linkage") tier("design") multiplicity("design") ///
    paperorder(-.8) specid("common_h_primary_observed") ///
...
_vrd_post_household_rd, ///
    posthandle(`rd_post') ///
    outvar(hh_link_rate_2017) ///
    outcomeid("D06") ///
    outlabel("Household member linkage rate") ///
    family("linkage") tier("design") multiplicity("design") ///
    paperorder(-.7) specid("common_h_household_link_rate") ///
    estimand("selection") usevar(rd_bc_design) scale(100) ///
    weighting("ccpp_equal") ///
    hvalue(${rd_common_h}) bvalue(${rd_common_b}) ///
    samplerule("selected_bc_household")
...
file open `contract_file' using ///
    "${rd_table_dir}/rd_2017_household_analysis_contract.csv", ///
    write replace text

file write `contract_file' ///
    "metric,value,status,interpretation" _n
file write `contract_file' ///
    `""unit","household","approved","One Census household linked deterministically to one RUV centro poblado""' _n
file write `contract_file' ///
...
file write `contract_file' ///
    `""weighting_sensitivity","household equal","required","Shows sensitivity to informative Census household counts and changes the population-weighted estimand""' _n
file write `contract_file' ///
...
file write `contract_file' ///
    `""selection_scope","INEI-assisted source households","limitation","Estimates do not automatically generalize to households absent from Census or unmatched communities""' _n
...
file open `registry_table' using ///
    "${rd_table_dir}/tab_rd_outcomes_21_registry_2017_household.tex", ///
    write replace text
...
file write `registry_table' "\caption{Registered Census 2017 household outcomes and construction rules}" _n
file write `registry_table' "\label{tab:rd_outcome_registry_2017_household} \\" _n
...
file open `first_stage_table' using ///
    "${rd_table_dir}/tab_rd_outcomes_22_sample_first_stage_2017_household.tex", ///
    write replace text
...
file write `first_stage_table' "\label{tab:rd_outcomes_first_stage_2017_household}" _n
...
file write `linkage_table' "\caption{Census 2017 household linkage and outcome-availability diagnostics}" _n
file write `linkage_table' "\label{tab:rd_outcomes_linkage_2017_household}" _n
file write `linkage_table' "\begin{tabular}{p{0.38\linewidth}rrrrr}" _n
file write `linkage_table' "\toprule" _n
file write `linkage_table' "Linkage outcome & Control mean & Discontinuity & 95\% robust CI & Households & CCPP \\" _n
file write `linkage_table' "\midrule" _n

preserve
use "`rd_results_final'", clear
keep if inlist(outcome_id, "D05", "D06")
sort paper_order
assert _N == 2
forvalues result_row = 1/`=_N' {
    local row_label = outcome_label[`result_row']
    ...
    local row_n : display %9.0fc ///
        n_eff_left[`result_row'] + n_eff_right[`result_row']
    local row_ccpp = ccpp_left[`result_row'] + ccpp_right[`result_row']
...
file open `main_table' using ///
    "${rd_table_dir}/tab_rd_outcomes_23_main_2017_household.tex", ///
    write replace text

file write `main_table' "\begin{table}[!htbp]" _n
file write `main_table' "\centering" _n
file write `main_table' "\scriptsize" _n
file write `main_table' "\caption{Collective reparations and registered Census 2017 household outcomes}" _n
file write `main_table' "\label{tab:rd_outcomes_main_2017_household}" _n
...
file open `secondary_table' using ///
    "${rd_table_dir}/tab_rd_outcomes_24_secondary_2017_household.tex", ///
    write replace text
...
file write `secondary_table' "\caption{Secondary and mechanism Census 2017 household fuzzy-RD outcomes}" _n
file write `secondary_table' "\label{tab:rd_outcomes_secondary_2017_household} \\" _n
...
file open `robustness_table' using ///
    "${rd_table_dir}/tab_rd_outcomes_25_robustness_2017_household.tex", ///
    write replace text
...
file write `robustness_table' "\label{tab:rd_outcomes_robustness_2017_household} \\" _n
...
program define _vrd_make_household_rdplot
    ...
    twoway ///
        ...
        note( ///
            "Notes: Unit is a linked Census household in the selected B/C geography; each RUV community has total weight one." ///
...
_vrd_make_household_rdplot, ///
    ...
    figure("${rd_figure_dir}/fig_rd_outcomes_53_first_stage_2016_household.png") ///
    firststage
...
forvalues outcome_index = 1/8 {
    ...
    local figure_number = `outcome_index' + 56
    ...
    local panel_label "Female members"
    if `outcome_index' == 2 local panel_label "Members age 15-29"
    if `outcome_index' == 3 local panel_label "Members in another CCPP"
    if `outcome_index' == 4 local panel_label "Secondary education"
    if `outcome_index' == 5 local panel_label "Employed members"
...
graph combine ///
    ...
    rows(3) imargin(tiny) ///
    title("Primary Census 2017 household RD plots", ///
        size(small) color(black)) ///
    subtitle("Common B-C design window; CCPP-equal reduced-form fits", ///
        size(vsmall) color(gs5)) ///
    note( ///
        "Notes: Unit is a linked Census household in the selected B/C geography; every RUV community receives total weight one." ///
...
graph export ///
    "${rd_figure_dir}/fig_rd_outcomes_54_primary_panels_2017_household.png", ///
    width(3600) replace
...
forvalues result_row = 1/`=_N' {
    ...
    local axis_label "Female members"
    if "`axis_outcome'" == "H02" local axis_label "Age 15--29"
    if "`axis_outcome'" == "H03" local axis_label "Moved CCPP"
    if "`axis_outcome'" == "H04" local axis_label "Secondary education"
    if "`axis_outcome'" == "H05" local axis_label "Employment"
    if "`axis_outcome'" == "H06" local axis_label "Health insurance"
    if "`axis_outcome'" == "H07" local axis_label "Disability"
    if "`axis_outcome'" == "H08" local axis_label "Core wellbeing"
...
twoway ///
    ...
    legend(off) ///
    note( ///
        "Notes: Unit is a linked Census household in the selected B/C geography; each RUV community receives total weight one." ///
        ...
        "The first stage passes the conservative F >= 20 gate; LATEs are still paired with reduced forms and Anderson-Rubin inference." ///
...
graph export ///
    "${rd_figure_dir}/fig_rd_outcomes_55_late_forest_2017_household.png", ///
    width(3000) replace
...
twoway ///
    ...
    legend(order(2 "h = 0.0050" 4 "h = 0.0075 (common)" ///
        ...
        region(lcolor(none))) ///
    note( ///
        "Notes: Unit is a linked Census household; each RUV community receives total weight one." ///
...
graph export ///
    "${rd_figure_dir}/fig_rd_outcomes_56_bandwidth_sensitivity_2017_household.png", ///
    width(3600) replace
...
local output_paths ///
    output/tables/rd_outcomes/rd_2017_household_results.csv ///
    output/tables/rd_outcomes/rd_2017_household_analysis_contract.csv ///
    output/tables/rd_outcomes/tab_rd_outcomes_21_registry_2017_household.tex ///
    output/tables/rd_outcomes/tab_rd_outcomes_22_sample_first_stage_2017_household.tex ///
    output/tables/rd_outcomes/tab_rd_outcomes_23_main_2017_household.tex ///
    output/tables/rd_outcomes/tab_rd_outcomes_24_secondary_2017_household.tex ///
    output/tables/rd_outcomes/tab_rd_outcomes_25_robustness_2017_household.tex ///
    output/tables/rd_outcomes/tab_rd_outcomes_32_linkage_2017_household.tex ///
    output/figures/rd_outcomes/fig_rd_outcomes_53_first_stage_2016_household.png ///
    output/figures/rd_outcomes/fig_rd_outcomes_54_primary_panels_2017_household.png ///
    output/figures/rd_outcomes/fig_rd_outcomes_55_late_forest_2017_household.png ///
    output/figures/rd_outcomes/fig_rd_outcomes_56_bandwidth_sensitivity_2017_household.png ///
    output/figures/rd_outcomes/fig_rd_outcomes_57_female_members_household.png ///
    output/figures/rd_outcomes/fig_rd_outcomes_58_age_15_29_household.png ///
    output/figures/rd_outcomes/fig_rd_outcomes_59_moved_ccpp_household.png ///
    output/figures/rd_outcomes/fig_rd_outcomes_60_secondary_education_household.png ///
    output/figures/rd_outcomes/fig_rd_outcomes_61_employment_household.png ///
    output/figures/rd_outcomes/fig_rd_outcomes_62_health_insurance_household.png ///
    output/figures/rd_outcomes/fig_rd_outcomes_63_disability_household.png ///
    output/figures/rd_outcomes/fig_rd_outcomes_64_wellbeing_core_household.png

tempname manifest_file
file open `manifest_file' using "${rd_2017_household_manifest}", ///
    write replace text
...
foreach output_path of local output_paths {
    ...
    if _rc {
        display as error "Expected 2017 household output was not created:"
        display as error "  `absolute_output'"
        file close `manifest_file'
        log close victimasrd_rd_2017_household
        exit 603
    }
    ...
    file write `manifest_file' ///
        `""`output_path'","`artifact_type'","13_census_2017_household_analysis.dta","`input_datasignature'","code/stata/pipeline/04e_census2017_household.do","${rd_run_id}","`output_checksum'","generated_unreviewed""' _n
}

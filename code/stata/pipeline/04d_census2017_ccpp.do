...
file write `registry_table' "\label{tab:rd_outcome_registry_2017_ccpp} \\" _n
...
file write `first_stage_table' "\label{tab:rd_outcomes_first_stage_2017_ccpp}" _n
...
file write `first_stage_table' "\parbox{0.97\linewidth}{\footnotesize \textit{Notes:} The outcome is cumulative collective-reparation receipt through 2016. Estimates are robust bias-corrected local-linear triangular-kernel discontinuities at the official B--C cutoff in the selected geography, with mass-point adjustment and district CR2 inference. The common design window is \(h=0.0075\) and bias window is \(b=0.0135\); the final row instead reports the treatment-based MSE selector. \(F_z\) is the squared robust RD \(z\) statistic and is descriptive, not the registered strength screen. The clustered local-IV Kleibergen--Paap screen is \(F>10\), with primary \(F=`kp_f_text'\). Fuzzy LATEs are also reported with reduced forms and weak-instrument-robust diagnostics. Source: RUV, CMAN, and INEI-assisted Census 2017.}" _n
...
file write `secondary_table' "\begin{minipage}{0.98\linewidth}\footnotesize\textit{Notes:} Each row is a separate fuzzy-RD estimate in the common \(h=0.0075\), \(b=0.0135\) window with local-linear triangular kernels, mass-point adjustment, and district CR2 inference. BH values control the false discovery rate within the declared outcome family. The internet row is an exploratory mechanism and is never an ordinary control. The GDP row uses the broader available B/C design sample and Seminario--Palomino estimates; other rows use Census-linked communities. The first-stage strength safeguard in Table~\ref{tab:rd_outcomes_first_stage_2017_ccpp} applies to every LATE. Source: RUV, CMAN, INEI-assisted Census 2017, and Seminario--Palomino.\end{minipage}" _n

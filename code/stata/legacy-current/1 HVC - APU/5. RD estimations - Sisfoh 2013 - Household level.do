...
putexcel set "$a\3 output\regressions\1 Huancavelica-Apurimac\$table.xls", sheet("Pooled CCPP") modify
	putexcel C3 = ("$table")
	sleep 3500
	...
	sleep 3500	
	putexcel I5 = ("RD no/covs. - Imposing rand. inf. window")
	sleep 3500	
	putexcel J5 = ("RD w/covs. - Imposing rand. inf. window")
	sleep 3500	
	putexcel K5 = ("RD w/covs. - Imposing rand. inf. window - Poly.2")
	sleep 3500
	putexcel C5 = ("Control mean")
	sleep 3500

	global cells D E F G H I J K
...
foreach y in $dep_hog {	
	...
	mean `y' if abs(index_cutBC)<0.015 & (dpto=="HUANCAVELICA" | dpto=="APURIMAC"), over(treat_13) 
	matrix mean_1=e(b)
	...
	*RD not including covariates - Optimal Bandwidht selection
	local p=1
		...
		fuzzy(treat_13) vce(cluster cluster_dist) bwselect(msecomb2) 			///
		kernel(triangular)
	...
	*RD including covariates -  Optimal Bandwidth selection
	...
	local l: word `p' of $cells	
		...
		(dpto=="HUANCAVELICA" | dpto=="APURIMAC"), ///
		fuzzy(treat_13) vce(cluster cluster_dist) bwselect(msecomb2) 			///
		kernel(triangular) covs($controls) p(1)
	...
	*RD including covariates - Optimal Bandwidth selection - 2nd grade poly.
	...
	local l: word `p' of $cells	
		...
		(dpto=="HUANCAVELICA" | dpto=="APURIMAC"), ///
		fuzzy(treat_13) vce(cluster cluster_dist) bwselect(msecomb2) 			///
		kernel(triangular) covs($controls) p(2)
	...
	*RD including covariates - Optimal Bandwidth selection - 3rd grade poly.
	...
	local l: word `p' of $cells	
		...
		(dpto=="HUANCAVELICA" | dpto=="APURIMAC"), ///
		fuzzy(treat_13) vce(cluster cluster_dist) bwselect(msecomb2) 			///
		kernel(triangular) covs($controls) p(3)
	...
	*RD including covariates - Optimal Bandwidth selection - 4th grade poly.
	...
	local l: word `p' of $cells	
		...
		(dpto=="HUANCAVELICA" | dpto=="APURIMAC"), ///
		fuzzy(treat_13) vce(cluster cluster_dist) bwselect(msecomb2) 			///
		kernel(triangular) covs($controls) p(4)

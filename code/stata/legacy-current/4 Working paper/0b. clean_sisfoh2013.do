...
use "$input_dir\1 Raw\ccpp_sisfoh2013.dta",clear
...
* reclink ubigeo_dist CCPP using "$a\2 data\communities_sisfoh.dta", idm(idm2) idu(idu)  gen(match) minscore(.9)
	...
	*Create id for fuzzy match done in data preparation code
	gen idu_sisfoh=_n
...
*---------------------------------------*	
	...
	egen 	edad_15mas			= rowtotal(sisf_edad3 sisf_edad4 sisf_edad5 sisf_edad6)
...
*---------------------------*	
	...
	egen		poverty2013_caracthh = rowmean(poverty2013_pared poverty2013_piso poverty2013_combust poverty2013_bienesriqueza)
	egen		poverty2013_servpub = rowmean(poverty2013_agua poverty2013_saneamiento poverty2013_alumbrado)
	egen		poverty2013_demogra = rowmean(poverty2013_seguro poverty2013_analfabeta poverty2013_educ poverty2013_desempleo)

	egen		poverty2013 = rowmean(poverty2013_caracthh poverty2013_servpub poverty2013_demogra)

	
save "$input_dir\2 Working\sisfoh2013_cleaned.dta", ///
	replace
...
use "$input_dir\1 Raw\sisfoh_hogar.dta", ///
	clear
...
save "$input_dir\2 Working\sisfoh_hogar_cleaned.dta", ///
	replace
...
use "$input_dir\2 Working\sisfoh_personas_cleaned.dta", clear
...
save "$input_dir\2 Working\sisfoh_persona_cleanedcollapse.dta", ///
	replace

*---------------------------------------------------------------------------*
*		IV.	Merge Sisfoh hogares with Sisfoh personas collapsed dataset		*
*---------------------------------------------------------------------------*
use "$input_dir\2 Working\sisfoh_persona_cleanedcollapse.dta", ///
	clear

	merge 1:1 ubigeo_ccpp SECUENCIA CONG NROVIV NHOGAR using "$input_dir\2 Working\sisfoh_hogar_cleaned.dta", ///
		keepusing(_all)
...
*	Type of fuel used to cook
	...
	* Different scores for urban/rural/capital
...
save "$input_dir\2 Working\sisfoh_hogarpersona_cleaned.dta", ///
	replace

*-----------------------------------------------------------*
*		V.	Sisfoh individual level - Pooled dataset		*
*-----------------------------------------------------------*
use "$input_dir\2 Working\sisfoh_personas_cleaned.dta", ///
	clear
...
*	(storage limit)
	...
	*replace lengua_nocastellano = 0 if IDIOMA01==4 
	...
	merge m:1 ubigeo_ccpp SECUENCIA CONG NROVIV NHOGAR using "$input_dir\2 Working\sisfoh_hogarpersona_cleaned.dta", ///
		keepusing(_all) ///
		gen(_merge_ind_both)
	/*
		Result                           # of obs.
...
save "$input_dir\2 Working\sisfoh_persona_both.dta", ///
	replace

* Create auxiliar dataset with new age distribution variables at ccpp level
	keep ubigeo_ccpp prop_edad*
	collapse (max) prop_edad*, by(ubigeo_ccpp)
save "$input_dir\2 Working\new_age_distribution2013.dta", ///
	replace
	
use "$input_dir\2 Working\sisfoh_persona_both.dta", ///
	clear
	
	keep ubigeo_ccpp prog_*
	collapse (sum) prog_*, by(ubigeo_ccpp)
	
save "$input_dir\2 Working\prog_distribution2013.dta", ///
	replace

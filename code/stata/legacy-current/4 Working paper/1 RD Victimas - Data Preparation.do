...
*-----------------------------------------------*
clear all
import excel "$input_dir\1 Raw\Community_index_inscritas.xlsx", ///
	sheet("Libro 02") ///
	firstrow ///
	clear
	...
	*Create new id for fuzzy match done later
	gen idm=_n

	*Clean the names of departamentos, provincias, distritos, and centros poblados
	...
	*Create identificator based on the dpto, prov, dist, ccpp names
	egen dpdc=concat(dpto2 prov2 dist2 ccpp2)
	sort dpdc
	bysort dpdc: gen rep=_N
	bysort dpdc: gen rep2=_n

save "$input_dir\2 Working\community_index_clean.dta", ///
	replace
...
*-----------------------------------------------*
clear all
import excel "$input_dir\1 Raw\comunidades_atendidas_por_ano.xlsx", ///
	sheet("Hoja1") ///
	firstrow ///
	clear
	...
	* Create id for fuzzy match done later
	gen idu_fin=_n

	* Clean names of dpto, prov, dist, ccpp
	gen 	dpto2=subinstr(dpto," ","",.)
	replace dpto2=subinstr(dpto2,"Á","A",.)
	replace dpto2=subinstr(dpto2,"É","E",.)
	replace dpto2=subinstr(dpto2,"Í","I",.)
...
save "$input_dir\2 Working\community_treated.dta", ///
	replace
...
use "$input_dir\2 Working\community_index_clean.dta", ///
	clear

* Exact math doesn't work becasue of community names that have different spelling
*merge 1:1 dpto2 prov2 dist2 ccpp2 using "$a\2 data\community_treated.dta"
	reclink ccpp2 dist2 prov2 dpto2 using "$input_dir\2 Working\community_treated.dta", ///
		idm(idm) ///
		idu(idu_fin)  ///
		gen(match) ///
		wnomatch(7 8 9 10) ///
	...
	drop U*
/*
	Note: After the fuzzy match, there are 102 financed communities that doesn't 
	appear in victimization	dataset. We don't have index score for those 
	communities
*/
	...
	* Create dummies for being financed (treatment) in each year
	gen 	treat_17=0
	...
	* Create clasification of financed projects (Infrastructure or Productive)
	...
	* Create dummy for Quintile A treated community until 2013
	gen		dum_a13=0
	replace dum_a13=1 if Nivel=="A" & treat_13==1	

	* Create variable with the proportion of Quintile A treated until 2023 within district
	bysort ubigeo_dist: egen	prop_a13=mean(dum_a13)
	drop 	dum_a13			

	* Create running variables centered at 0 (Quintiles cutoff points)
	...
	* Create dummy for sample in study (Huancavelica, Apurimac, Huancayo (Junin), and La Convencion (Cusco)
	gen		sample2 = 0
	replace sample2 = 1 if dpto=="HUANCAVELICA" | dpto=="APURIMAC" | prov=="HUANCAYO" | prov=="LA CONVENCION"

save "$input_dir\2 Working\community_index_treated.dta", ///
	replace

*-------------------------------------------------------------------*
*		Merge treatment dataset with Sisfoh 2013 at CCPP level		*
*-------------------------------------------------------------------*
use "$input_dir\2 Working\community_index_treated.dta", ///
	clear
	
	*merge 1:1 dpto prov dist ccpp using "$a\2 data\sisfoh2013_cleaned.dta", gen(_merge2)

	* Fuzzy match with Sisfoh data (Main 2013 outcomes)
	reclink ccpp2 ubigeo_dist using "$input_dir\2 Working\sisfoh2013_cleaned.dta", ///
		idm(idm) ///
		idu(idu_sisfoh) ///
		gen(match) ///
		wnomatch(5 10) ///
	...
	* Dropping repeated values after fuzzy match (Note: Should check 1 by 1 to correctly identify which to keep)
	...
	*save "$input_dir\3 Coded\community_index_treated_sisfohccpp.dta", ///
	*	replace
...
*-------------------------------------------*
	...
	* Merge with data on APRA-elected major in 2006 for districts in Huancavelica and Apurimac
	merge m:1 ubigeo_dist using "$input_dir\1 Raw\distritos_apristas_hcvapu.dta", ///
		gen(_merge_apra)
...
*-------------------------------------------------------*
	...
	* Add georeferenced data (location + calculated distances)
	merge 1:1 ubigeo_ccpp using "$input_dir\1 Raw\ccpp_georef_dist.dta", ///
		gen(_merge_dist) ///
		keepusing(longitude latitude cap_dist longitude_capital latitude_capital distancia_capital cap_reg latitude_capital_region longitude_capital_region distancia_capital_region)
...
*-----------------------------------------------*
	...
	* Add indicator for mining district in 2017
	merge m:1 ubigeo_dist using "$input_dir\1 Raw\distritos_mineros.dta", ///
		keepusing(dist_minero) ///
		gen(_merge_minero)
...
*-------------------------------------------*
	...
	* Add 2017 comunity population and household numbers, natural region and altitude
	merge 1:1 ubigeo_ccpp using "$input_dir\1 Raw\pob_ccpp_2017.dta", ///
		keepusing(pob_ccpp_2017 viv_ccpp_2017 natural_region altitude_msnm)
	drop if _merge==2
	drop _merge

*-----------------------------------------------------------*
*		Merge with public expenditure execution dataset		*
*-----------------------------------------------------------*
	
	*Add public finance variables (total budget and execution) at district level
	merge m:1 ubigeo_dist using "$input_dir\1 Raw\ejecucion_presupuesto.dta"
	drop if _merge==2
	drop _merge
...
*-------------------------------------------*
	...
	merge 1:1 ubigeo_ccpp using "$input_dir\2 Working\ece_2009_2015.dta", ///
		gen(_merge_ece)
	drop if _merge==2
...
*-----------------------------------------------*
	...
	merge 1:1 ubigeo_ccpp using "$input_dir\2 Working\CE_2010_2015.dta", ///
		gen(_merge_ce)
	drop if _merge==2
	local vars serv_ie_alim serv_ie_salud serv_ie_otro serv_ie_ninguno ///
		num_ie_estatal num_ie_privada tipo_ie_unimulti tipo_ie_polimulti ///
		ie_secundaria ie_primaria num_ie tot_alum tot_alum_m tot_alum_h ///
		tot_alum_primaria tot_alum_m_primaria tot_alum_h_primaria ///
		tot_alum_secundaria tot_alum_m_secundaria tot_alum_h_secundaria ///
		tot_alum_estatal tot_alum_m_estatal tot_alum_h_estatal ///
		tot_alum_privada tot_alum_m_privada tot_alum_h_privada
...
*-------------------------------------------------------------------*
	...
	merge 1:1 ubigeo_ccpp using "$input_dir\2 Working\censo2007_cleaned.dta", ///
		gen(_merge_censo)
...
*-----------------------------------------------------------*
	merge 1:1 ubigeo_ccpp using "$input_dir\2 Working\new_age_distribution2013.dta", ///
		gen(_merge_newage)
	...
	*merge 1:m ubigeo_ccpp using "$input_dir\2 Working\prog_distribution2013.dta", gen(_merge_sisfoh)
...
*-----------------------------------*		
	*---------------------------------------------------*
	*		Merge with Cenagro - Producer level 		*
	*---------------------------------------------------*
	use "$input_dir\3 Coded\community_index_treated_sisfohccpp.dta", ///
		clear

		merge 1:m ubigeo_ccpp using "$input_dir\2 Working\cenagro_producer.dta", ///
			gen(_merge_cenagroprod)
		drop if _merge_cenagroprod==2
		drop _merge_cenagroprod
		
	save "$input_dir\3 Coded\community_index_treated_cenagroprod.dta", ///
		replace

	*---------------------------------------------------*
	*		Merge with Cenagro - Community level 		*
	*---------------------------------------------------*
	use "$input_dir\3 Coded\community_index_treated_sisfohccpp.dta", ///
		clear

		merge 1:1 ubigeo_ccpp using "$input_dir\2 Working\cenagro_ccpp.dta", ///
			gen(_merge_cenagroprod)
		drop if _merge_cenagroprod==2
	...
	save "$input_dir\3 Coded\community_index_treated_cenagroccpp.dta", ///
		replace

	*-----------------------------------------------*
	*		Merge with Minsa - Community level 		*
	*-----------------------------------------------*
	use "$input_dir\3 Coded\community_index_treated_sisfohccpp.dta", clear

		merge 1:1 ubigeo_ccpp using "$input_dir\2 Working\minsa_ccpp.dta", ///
			gen(_merge_minsa)
		drop if _merge_minsa==2
		
		local vars num_cs categ_0 categ_i1 categ_i2 categ_i3 categ_i4 categ_ii1 ///
			categ_ii2 categ_iie categ_iii1 categ_iii2 categ_iiie tipo_institucion
		foreach x of local vars {
	...
	save "$input_dir\3 Coded\community_index_treated_minsaccpp.dta", ///
		replace
...
use "$input_dir\2 Working\censo2007_cleaned.dta", ///
	clear
	
	collapse (sum) pob_2007=pob_ccpp_2007 hog_2007=hog_ccpp_2007 (mean) poverty*, ///
		by(ubigeo_dist)

save "$input_dir\2 Working\censo2007_distrito.dta", ///
	replace
...
use "$input_dir\1 Raw\pob_ccpp_2017.dta", ///
	clear
	
	gen ubigeo_dist=substr(ubigeo_ccpp,1,6)
	collapse (sum) pob_2017=pob_ccpp_2017 hog_2017=viv_ccpp_2017 (mean) altitude_msnm, ///
		by(ubigeo_dist)

save "$input_dir\2 Working\pob2017_distrito.dta", ///
	replace
...
use "$input_dir\3 Coded\community_index_treated_sisfohccpp.dta", ///
	...
	collapse (firstnm) dpto prov cluster_prov sample2 ///
	(max) dist_minero ///
	(sum) nivel_* num_ccpp* num_pob* num_hog* ///
	(mean) distancia_capital_region index_cutBC index_victim pim* dev* budget_exec*, 		///
		by(ubigeo_dist)

	*Dataset with number of CCPP in each district
	merge 1:1 ubigeo_dist using "$input_dir\1 Raw\ccpp_xdistrito.dta"
	drop if _merge!=3
	drop _merge
...
*	% of victimized and treated communities
	...
	merge 1:1 ubigeo_dist using "$input_dir\2 Working\censo2007_distrito.dta"
	drop if _merge!=3
	drop _merge
	merge 1:1 ubigeo_dist using "$input_dir\2 Working\pob2013_distrito.dta"
	drop if _merge!=3
	drop _merge
	merge 1:1 ubigeo_dist using "$input_dir\2 Working\pob2017_distrito.dta"
	drop if _merge==2
	drop _merge
...
*	Merge with Cenagro
	merge 1:1 ubigeo_dist using "$input_dir\2 Working\cenagro_district.dta"
	drop if _merge==2
...
*	Merge with Minsa
	merge 1:1 ubigeo_dist using "$input_dir\2 Working\minsa_district.dta"
	drop if _merge==2
	drop _merge

*	Merge with Minedu
	merge 1:1 ubigeo_dist using "$input_dir\2 Working\minedu_district.dta"
	drop if _merge==2
	drop _merge

*	Merge with Social Conflicts dataset
	merge 1:1 ubigeo_dist using "$input_dir\2 Working\conflict_clean.dta"
	drop if _merge==2
...
*	Merge with ONPE
	merge 1:1 ubigeo_dist using "$input_dir\2 Working\onpe_2006_2016.dta"
	drop _merge

save "$input_dir\3 Coded\community_index_treated_district.dta", ///
	replace
...
save "$input_dir\2 Working\censo2007_distrito_NoCapDist.dta", ///
	replace
...
use "$input_dir\1 Raw\pob_ccpp_2017.dta", ///
	...
	gen		cod_cap = substr(ubigeo_ccpp,-4,4)
	drop if cod_cap=="0001"
	collapse (sum) pob_2017=pob_ccpp_2017 hog_2017=viv_ccpp_2017 ///
	(mean) altitude_msnm, ///
	by(ubigeo_dist)
	
save "$input_dir\2 Working\pob2017_distrito_NoCapDist.dta", ///
	replace
...
use "$input_dir\3 Coded\community_index_treated_sisfohccpp.dta", ///
	...
	collapse (firstnm) dpto prov cluster_prov sample2 (sum) nivel_* num_ccpp* num_pob* num_hog* ///
	(max) dist_minero ///
	(mean) distancia_capital_region index_cutBC index_victim pim* dev* budget_exec*, 		///
	by(ubigeo_dist)
...
*	% of victimized and treated communities
	...
	gen	ubigeo_dpto = substr(ubigeo_dist,1,2)
	encode ubigeo_dpto, gen(cluster_dpto)
	merge 1:1 ubigeo_dist using "$input_dir\2 Working\censo2007_distrito_NoCapDist.dta"
	drop if _merge!=3
	drop _merge
	merge 1:1 ubigeo_dist using "$input_dir\2 Working\pob2013_distrito_NoCapDist.dta"
	drop if _merge!=3
	drop _merge
	merge 1:1 ubigeo_dist using "$input_dir\2 Working\pob2017_distrito_NoCapDist.dta"
	drop if _merge==2
	drop _merge
...
*	Merge with Cenagro
	merge 1:1 ubigeo_dist using "$input_dir\2 Working\cenagro_district_NoCapDist.dta"
	drop if _merge==2
	local vars total_trab_mujeres total_trab_hombres riego_gravedad riego_aspersion prom_trab_mujeres prom_parc prom_num_vacuno prom_num_ovino prom_num_cultivos prom_num_alpaca prom_area_parc prom_area_agricola num_productores num_productor_m num_parc num_cultivos ganaderia area_parc area_agricola
	foreach x of local vars {
		replace `x'=0 if _merge==1
...
*	Merge with Minedu
	merge 1:1 ubigeo_dist using "$input_dir\2 Working\minedu_district_NoCapDist.dta"
	drop if _merge==2
	drop _merge
...
save "$input_dir\3 Coded\community_index_treated_district_NoDistCap.dta", ///
	replace
...
use "$input_dir\3 Coded\community_index_treated_sisfohccpp.dta", ///
	...
	merge 1:m ubigeo_ccpp using "$input_dir\2 Working\sisfoh_hogarpersona_cleaned.dta", ///
		gen(_merge_sisf_pers)

/*
    Result                           # of obs.
    -----------------------------------------
    not matched                     6,193,776
        from master                       148  (_merge_sisf_pers==1)
        from using                  6,193,628  (_merge_sisf_pers==2)

    matched                           415,942  (_merge_sisf_pers==3)
    -----------------------------------------
*/
	drop if _merge_sisf_pers!=3
	drop _merge_sisf_pers

save "$input_dir\3 Coded\community_index_treated_sisfohhogpers.dta", ///
	replace
...
merge 1:m ubigeo_ccpp using "$input_dir\2 Working\sisfoh_persona_both.dta", ///
	gen(_merge_sisf_both)
...
save "$input_dir\3 Coded\community_index_treated_sisfohpers_both.dta", ///
	replace
...
merge 1:m ubigeo_ccpp using "$input_dir\2 Working\sisfoh_persona_women.dta", ///
	gen(_merge_sisf_women)
...
save "$input_dir\3 Coded\community_index_treated_sisfohpers_women.dta", ///
	replace
...
merge 1:m ubigeo_ccpp using "$input_dir\2 Working\sisfoh_persona_men.dta", ///
	gen(_merge_sisf_men)
...
save "$input_dir\3 Coded\community_index_treated_sisfohpers_men.dta", ///
	replace

...
1. **Assignment-effect heterogeneity.** `rdhte` estimates whether the sharp RD
   discontinuity in an outcome varies with a predetermined moderator. These
   are reduced-form or assignment effects. They remain interpretable without
   dividing by a first stage, subject to local support and the RD assumptions.
2. **Fuzzy-LATE heterogeneity.** A pooled, fully interacted local-linear 2SLS
   ...
   interaction is the formal difference or slope in the local complier effect.
3. **Conditional local effects.** The same pooled IV model is used to report
...
`rdhte` is not used as a fuzzy-RD estimator. Its current Stata implementation
...
family within a wave and level and receive Holm family-wise adjustment.
Secondary moderator interactions receive Benjamini--Hochberg adjustment within
the wave and level. Machine-readable files retain raw and adjusted p-values,
all failed estimates, and all interpretation gates.

The main figures emphasize assignment-effect heterogeneity when the fuzzy
interaction instruments are weak. A non-significant result is not evidence of
homogeneous effects when confidence intervals are wide.
...
For continuous moderators, local support requires at least 10 analysis units
on each side. For binary moderators, each moderator-by-side cell must contain
at least 10 analysis units. Results that fail support remain in the diagnostic
files but are not presented as credible causal heterogeneity. The gate may not
be weakened after seeing results.
